...
const CivilianGameRoom: React.FC<CivilianGameRoomProps> = ({ 
  category, 
  domain, 
  socket, 
  socketId, 
  ...
  const [sharedRunStatus, setSharedRunStatus] = useState<any[] | null>(null)
  ...
  useEffect(() => {
    ...
    const handleRunStatusUpdate = (data: any) => {
      setSharedRunStatus(data.realResults)
    }
    ...
    <>
      ...
      <div className="border-r-4 border-black bg-[#2d2d2d] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4">
          <h2 className="bg-green-600 text-white px-2 py-1 mb-3 text-sm font-['Press_Start_2P']">TEST CASES</h2>
          <ul className="space-y-3">
            {testCases.length === 0 ? (
              ...
              testCases.slice(0, 4).map((tc, index) => {
                // Match against sharedRunStatus (broadcast) or runResults (local)
                const result = sharedRunStatus?.find((r: any) => r.input === tc.input)
